...
import { Trophy, Mail, Lock, ArrowRight, Loader2, Sparkles, User, Target } from "lucide-react";
...
export default function RegisterFanPage() {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success("Kullanıcı kaydı başarılı! 🎉 Hoş geldin.");
                ...
                <div className="space-y-8">
                    <div className="text-center space-y-4">
                        <div className="flex justify-center">
                            <div className="w-16 h-16 bg-indigo-600 rounded-3xl flex items-center justify-center text-white shadow-xl shadow-indigo-600/20 rotate-3">
                                <Target className="w-8 h-8 fill-white" />
                            </div>
                        </div>
                        <div className="space-y-2">
                            <h1 className="text-4xl font-black tracking-tighter text-white uppercase leading-none">
                                <ShinyText text="Yeni Bir" className="block" />
                                <span className="text-indigo-400">Oyuncu!</span>
                            </h1>
                            <p className="text-slate-400 font-bold text-sm italic font-serif">
                                Görevleri tamamla, ödülleri kap.
                            </p>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleRegister} className="space-y-5">
                        <div className="space-y-2 text-left">
                            ...
                            <div className="relative group">
                                ...
                                <input
                                    ...
                                    className="w-full h-14 bg-white/5 border-2 border-white/5 rounded-2xl pl-12 pr-4 text-sm font-bold focus:outline-none focus:border-indigo-500/50 focus:bg-white/10 transition-all shadow-sm text-white"
                                    value={name}
                        ...
                        <Button
                            type="submit"
                            className="w-full h-16 bg-indigo-600 rounded-2xl text-xl font-black text-white shadow-xl shadow-indigo-600/20 hover:scale-[1.02] active:scale-95 transition-all group/btn"
                            disabled={loading}
                        >
                            {loading ? (
                                ...
                                <span className="flex items-center gap-2">
                                    Maceraya Katıl <ArrowRight className="h-6 w-6 group-hover/btn:translate-x-1 transition-transform" />
                                </span>
                            )}
                        </Button>
                    </form>
